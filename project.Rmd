...
```{r intersect, eval=FALSE}
# Adding the green areas to the map
map <- map %>% addPolygons(data = green_areas, color = "green", fill = "green", weight = 2, group = "Green areas")
...
```{r intersect, eval=FALSE}
#Adding second layer to leaflet map
#map <- map %>% addPolygons(data = school_districts, color = "red", fill= "blue", weight = 2, group = "School districts")
#map
```
...
joined_data <- school_districts_valid %>%
...
map <- map %>% addPolygons(data = joined_data, color = "red", fill= "blue", weight = 2, label=joined_data$freq, , group = "School districts")
map
```

```{r}
# Creating dataframe with only the park frequency per district and the name of the school district
parks_per_district <- subset(joined_data, select = c("freq", "enhedsnavn"))
