...
map <- map %>% addPolygons(data = green_areas, color = "green", fill = "green", weight = 2)
...
map <- map %>% addPolygons(data = school_districts, color = "red", fill= "blue", weight = 2)
map
```
...
```{r}
#manully tranforming 'dataset' into dataframe
df1 <- data.frame(
  id = c(1:56),
...
```{r}
# Adding school districts with number of parks as label. Label displays when hoovering over district
